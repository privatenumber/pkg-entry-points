...
describe('Node.js resolve behavior', () => {
	...
	test('blocks path with condition', async () => {
		...
		expect(await assertSubpath('pkg/b', [])).toMatch('/lib/b.mjs');
		await expect(() => assertSubpath('pkg/a', ['a'])).rejects.toThrow('ERR_PACKAGE_PATH_NOT_EXPORTED');
	...
	test('Doesnt match file with * in name', async () => {
		...
		await expect(() => assertSubpath('pkg/a', ['a'])).rejects.toThrow('ERR_MODULE_NOT_FOUND');
